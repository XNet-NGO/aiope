...
class FaceEngine(
    ...
    companion object {
        ...
        /** Cosine-similarity threshold for a confident identity match. */
        const val MATCH_THRESHOLD = 0.45f
        /** Top match must beat the runner-up (different person) by at least this margin. */
     ...
     *
     * To avoid confusing similar people, the top label must (a) meet [MATCH_THRESHOLD] and
     * (b) beat the second-best DIFFERENT label by at least [MATCH_MARGIN]. Otherwise the result
     * is ambiguous -> null (unidentified).
     */
    fun identify(bitmap: Bitmap, enrolled: List<Pair<String, FloatArray>>): Pair<String, Float>? {
        ...
        // Best (max) cosine per label across all its angle samples.
        ...
        Log.d(
            TAG,
            "identify: top=${top.key} score=${top.value} runnerUp=$runnerUp margin=$margin " +
                "(threshold=$MATCH_THRESHOLD, marginReq=$MATCH_MARGIN)",
        )

        val confident = top.value >= MATCH_THRESHOLD && (ranked.size == 1 || margin >= MATCH_MARGIN)
        return if (confident) top.key to top.value else null
    }
    ...
    private fun embed(aligned: Bitmap): FloatArray? {
        ...
        // ArcFace (garavv/arcface-onnx) expects NHWC: [1, 112, 112, 3].
        val input = bitmapToRgbNhwc(aligned, ALIGN_SIZE, ALIGN_SIZE)
        val tensor = OnnxTensor.createTensor(
            e, FloatBuffer.wrap(input), longArrayOf(1, ALIGN_SIZE.toLong(), ALIGN_SIZE.toLong(), 3),
        )
        tensor.use { t ->
            s.run(mapOf(inputName(s) to t)).use { result ->

...
class ToolExecutor(
  ...
  fun buildToolDefs() = (if (getAgentMode().disablesAllTools) emptyList() else listOf(
    td("run_sh", "Execute an Android shell command. timeout seconds: 10-30 for quick commands, 60-120 for network, 300-600 for builds.", """{"type":"object","properties":{"command":{"type":"string","description":"Shell command to execute"},"timeout":{"type":"integer","description":"Timeout in seconds (default 300)"}},"required":["command"]}"""),    td("run_proot", "Run a command in the Alpine Linux proot environment (apk, python, gcc, etc). Set timeout for the command.", """{"type":"object","properties":{"command":{"type":"string","description":"Command to execute in Alpine"},"timeout":{"type":"integer","description":"Timeout in seconds (default 300; 60 for apk, 600 for builds)"}},"required":["command"]}"""),
    ...
    td("open_intent", "Open a URL, app, map, dialer, or navigation intent. Supports https://, geo:, google.navigation:q=, tel:, mailto:, etc.", """{"type":"object","properties":{"uri":{"type":"string","description":"URI to open (https://, geo:, google.navigation:q=, tel:, mailto:, ...)"}},"required":["uri"]}"""),
    td("fetch_url", "Fetch a URL. Returns extracted text and any images as ![alt](url) markdown (include them in your response). mode 'raw' for raw response; offset/limit paginate.", """{"type":"object","properties":{"url":{"type":"string","description":"URL to fetch"},"mode":{"type":"string","description":"'raw' or 'text' (default)"},"offset":{"type":"integer","description":"Char offset for pagination"},"limit":{"type":"integer","description":"Max chars to return"}},"required":["url"]}"""),
    td("query_data", "Query live real-time data. Returns JSON and any images as ![alt](url) markdown (include them in your response). Uses device GPS for location queries. 'extra' = search query / station ID / breed ID per category. Categories: ${fetchDataCategories()}", """{"type":"object","properties":{"category":{"type":"string","description":"Data category"},"extra":{"type":"string","description":"Optional: search query, station ID, or breed ID"}},"required":["category"]}"""),
    td("search_location", "Search for a place, address, landmark, or business/amenity. For nearby searches, call get_location first. Handles addresses, landmarks, cities, and amenity searches.", """{"type":"object","properties":{"query":{"type":"string","description":"What to search for, e.g. 'Eiffel Tower', 'pizza in Boise, ID', 'gas station'"}},"required":["query"]}"""),
    td("search_web", "Search the web for current information, news, or facts. Returns titles, URLs, and snippets. Use for recent events or anything needing up-to-date info.", """{"type":"object","properties":{"query":{"type":"string","description":"Search query"}},"required":["query"]}"""),
    ...
    td("orchestrate", "Run a multi-agent DAG pipeline. Each stage dispatches a named roster agent; stages without depends_on run in parallel; results flow to dependent stages.", """{"type":"object","properties":{"task":{"type":"string","description":"Overall task description"},"stages":{"type":"array","description":"Pipeline stages","items":{"type":"object","properties":{"name":{"type":"string","description":"Unique stage name"},"agent":{"type":"string","description":"Architect, Coder, Researcher, QA, DevOps, Security, Writer, or Reviewer"},"prompt":{"type":"string","description":"Task for this stage"},"depends_on":{"type":"array","items":{"type":"string"},"description":"Stage names this depends on"}},"required":["name","agent","prompt"]}}},"required":["task","stages"]}"""),
    td("todo_write", "Write/update your persistent task list. Plan before a multi-step task, then update item status as you work. Full replace unless merge=true (upsert by id).", """{"type":"object","properties":{"todos":{"type":"array","description":"Todo items","items":{"type":"object","properties":{"id":{"type":"string","description":"Short stable id"},"content":{"type":"string"},"status":{"type":"string","enum":["pending","in_progress","completed","cancelled"]}},"required":["id","content"]}},"merge":{"type":"boolean","description":"true = upsert by id; false/omit = full replace"}},"required":["todos"]}"""),
    td("todo_read", "Read your current persistent todo list grouped by status, with counts.", """{"type":"object","properties":{}}"""),
    td("edit_file", "Find/replace in a text file (prefer over write_file for small changes). Pass exact old_string incl. indentation. Errors if not found or matches multiple times unless replace_all=true.", """{"type":"object","properties":{"path":{"type":"string"},"old_string":{"type":"string","description":"Exact text to find"},"new_string":{"type":"string","description":"Replacement (empty deletes)"},"replace_all":{"type":"boolean","description":"Replace all occurrences"}},"required":["path","old_string","new_string"]}"""),
    td("search_files", "Search a directory tree. target='content' (default) greps file contents by regex (path:line:text); target='files' matches file names. Optional glob narrows files. Skips .git/node_modules/build.", """{"type":"object","properties":{"path":{"type":"string","description":"Directory to search"},"pattern":{"type":"string","description":"Regex pattern"},"target":{"type":"string","enum":["content","files"]},"glob":{"type":"string","description":"Optional filename glob like '*.md'"},"limit":{"type":"integer","description":"Max results (default 50)"}},"required":["path","pattern"]}"""),
    td("http_request", "Call an HTTP API: method, headers, body, timeout. Returns status, key headers, and body (~20KB truncated). Prefer fetch_url for reading web pages.", """{"type":"object","properties":{"url":{"type":"string"},"method":{"type":"string","enum":["GET","POST","PUT","PATCH","DELETE"],"description":"Default GET"},"headers":{"type":"object","description":"e.g. {'Authorization':'Bearer ...'}"},"body":{"type":"string","description":"Request body (POST/PUT/PATCH/DELETE)"},"timeout_seconds":{"type":"integer","description":"Default 30, max 300"}},"required":["url"]}"""),
    td("schedule_task", "Schedule the agent to run a prompt automatically in the background. schedule_type: 'interval' (every interval_value interval_unit min|hour|day), 'daily'/'weekly'/'monthly' (fire at time_hour:time_minute; weekly uses days_of_week '1,4' where 1=Sun; monthly uses day_of_month 1-28), or 'once' (~60s later). max_runs caps executions (0=unlimited). tools: comma-separated subset allowed during runs. Each run reports via notification.", """{"type":"object","properties":{"prompt":{"type":"string","description":"What the scheduled agent should do each run"},"schedule_type":{"type":"string","enum":["once","interval","daily","weekly","monthly"]},"interval_value":{"type":"integer","description":"Every N units (interval type, default 30)"},"interval_unit":{"type":"string","enum":["min","hour","day"]},"time_hour":{"type":"integer","description":"Hour 0-23 (daily/weekly/monthly)"},"time_minute":{"type":"integer","description":"Minute 0-59"},"days_of_week":{"type":"string","description":"Weekly: comma list 1=Sun..7=Sat"},"day_of_month":{"type":"integer","description":"Monthly: day 1-28"},"max_runs":{"type":"integer","description":"Stop after N runs (0=unlimited)"},"tools":{"type":"string","description":"Comma-separated tool subset for background runs"}},"required":["prompt","schedule_type"]}"""),
    ...
    defs
  }).filter { toolStore.isToolEnabledForMode(it.name, getAgentMode()) }
  ...
  suspend fun execute(name: String, args: Map<String, Any?>): String {
    ...
    // Per-mode availability is enforced by the allow-list in buildToolDefs (a
    // tool not allowed in the current mode is never offered to the model). We do
    // NOT re-gate here — doing so previously blocked tools that ARE allowed in a
    // mode (e.g. send_sms in Chat), causing "offered but rejected" failures.
    return when (name) {
      "run_sh" -> {

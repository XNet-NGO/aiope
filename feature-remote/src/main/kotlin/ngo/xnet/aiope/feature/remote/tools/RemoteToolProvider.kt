...
class RemoteToolProvider @Inject constructor(
  ...
  override fun buildToolDefs(): List<RemoteToolBridge.ToolDef> = listOf(
    ...
    RemoteToolBridge.ToolDef(
      ...
      description = "Start a browser on a remote server for automation. engine 'firefox' or 'chrome'. Both engines can share the user's real logged-in session: they drive a persistent AIOPE profile SEEDED from the user's real profile (cookies/auth/tokens carry over; the real profile is never driven or modified). Firefox ALWAYS uses this shared profile. Chrome uses it when share_session=true (otherwise a clean throwaway profile). For either engine, refresh=true pulls in fresh auth from the user's real profile (auth-only, keeps the agent's accumulated session data) and reseed=true fully resets the AIOPE profile from the user's real profile (recovery); reseed closes any running browser on the source profile first. By default the browser runs HEADED (visible, attaches to the server's active graphical session) and auto-falls-back to HEADLESS when no active display exists; pass mode='headed' or mode='headless' to force one (relay the user's explicit choice if they state it). Requires an active ssh session.",
      parameters = """{"type":"object","properties":{"server":{"type":"string","description":"Server name or ID"},"engine":{"type":"string","enum":["firefox","chrome"],"description":"Browser engine (default firefox)"},"share_session":{"type":"boolean","description":"Chrome: use the persistent AIOPE profile seeded from the user's login (default false). Firefox always shares — this flag is a no-op for Firefox."},"refresh":{"type":"boolean","description":"Either engine: auth-only refresh from the user's real profile (keeps the agent's accumulated session data)"},"reseed":{"type":"boolean","description":"Either engine: full reset of the AIOPE profile from the user's real profile (recovery)"},"mode":{"type":"string","enum":["headed","headless"],"description":"Force display mode. Omit for default (headed with auto-fallback to headless). Relay the user's explicit choice when they state one."}},"required":["server"]}""",
    ),
    RemoteToolBridge.ToolDef(

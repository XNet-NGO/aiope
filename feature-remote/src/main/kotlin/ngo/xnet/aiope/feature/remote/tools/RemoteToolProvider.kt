...
class RemoteToolProvider @Inject constructor(
  ...
  override fun buildToolDefs(): List<RemoteToolBridge.ToolDef> = listOf(
    ...
    RemoteToolBridge.ToolDef(
      ...
      description = "Start a browser on a remote server for automation. engine 'firefox' or 'chrome'. For Chrome, share_session=true uses AIOPE's persistent Chrome profile (seeded once from the user's real logged-in profile so cookies/auth carry over; the user's real profile is never driven or modified). This profile persists across sessions and accumulates the agent's own logins. Use refresh=true to pull in new auth from the user's real profile (auth-only, keeps the agent's session data) or reseed=true to fully reset the profile from the user's real profile (recovery). By default the browser runs HEADED (visible, attaches to the server's active graphical session) and auto-falls-back to HEADLESS when no active display exists; pass mode='headed' or mode='headless' to force one (relay the user's explicit choice if they state it). Requires an active ssh session.",
      parameters = """{"type":"object","properties":{"server":{"type":"string","description":"Server name or ID"},"engine":{"type":"string","enum":["firefox","chrome"],"description":"Browser engine (default firefox)"},"share_session":{"type":"boolean","description":"Chrome: use the persistent AIOPE profile seeded from the user's login (default false)"},"refresh":{"type":"boolean","description":"Chrome: auth-only refresh from the user's real profile (keeps agent session data)"},"reseed":{"type":"boolean","description":"Chrome: full reset of the AIOPE profile from the user's real profile (recovery)"},"mode":{"type":"string","enum":["headed","headless"],"description":"Force display mode. Omit for default (headed with auto-fallback to headless). Relay the user's explicit choice when they state one."}},"required":["server"]}""",
    ),
    RemoteToolBridge.ToolDef(

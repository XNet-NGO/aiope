...
class RemoteToolProvider @Inject constructor(
  ...
  override fun buildToolDefs(): List<RemoteToolBridge.ToolDef> = listOf(
    ...
    RemoteToolBridge.ToolDef(
      ...
      description = "Start a browser on a remote server for automation. engine 'firefox' or 'chrome' — the two engines are 1:1 (identical tools and flags). Both can share the user's real logged-in session: set share_session=true to drive a persistent AIOPE profile SEEDED from the user's real profile (cookies/auth/tokens carry over; the real profile is never driven or modified), otherwise a clean throwaway profile is used. For either engine, refresh=true pulls in fresh auth from the user's real profile (auth-only, keeps the agent's accumulated session data; implies share_session) and reseed=true fully resets the AIOPE profile from the user's real profile (recovery; closes any running browser on the source first; implies share_session). By default the browser runs HEADED (visible, attaches to the server's active graphical session) and auto-falls-back to HEADLESS when no active display exists; pass mode='headed' or mode='headless' to force one (relay the user's explicit choice if they state it). Requires an active ssh session.",
      parameters = """{"type":"object","properties":{"server":{"type":"string","description":"Server name or ID"},"engine":{"type":"string","enum":["firefox","chrome"],"description":"Browser engine (default firefox)"},"share_session":{"type":"boolean","description":"Both engines: drive the persistent AIOPE profile seeded from the user's login for auth sharing (default false = throwaway profile)"},"refresh":{"type":"boolean","description":"Both engines: auth-only refresh from the user's real profile (keeps agent session data; implies share_session)"},"reseed":{"type":"boolean","description":"Both engines: full reset of the AIOPE profile from the user's real profile (recovery; implies share_session)"},"mode":{"type":"string","enum":["headed","headless"],"description":"Force display mode. Omit for default (headed with auto-fallback to headless). Relay the user's explicit choice when they state one."}},"required":["server"]}""",
    ),
    RemoteToolBridge.ToolDef(
